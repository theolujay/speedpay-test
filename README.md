...
1. Call `POST /v1/account/deposit` to get `authorization_url` and `reference`.
...
curl http://localhost:8000/v1/account/balance \
  -H "Authorization: Bearer <access_token>"

# Withdraw
curl -X POST http://localhost:8000/v1/account/withdraw \
  -H "Authorization: Bearer <access_token>" \
  -H "Content-Type: application/json" \
  -d '{"amount": "500.00"}'

# Transfer to another user
curl -X POST http://localhost:8000/v1/account/transfer \
  -H "Authorization: Bearer <access_token>" \
  -H "Content-Type: application/json" \
...
1. Call `POST /v1/auth/token` to get `{access, refresh}`.
...
### Account -- `/v1/account/` (JWT required)
...
### Admin -- `/v1/admin/` (JWT + admin required)
